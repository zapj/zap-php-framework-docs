import { viteBundler } from '@vuepress/bundler-vite'
import { defaultTheme } from '@vuepress/theme-default'
import { defineUserConfig } from 'vuepress'

export default defineUserConfig({
  bundler: viteBundler(),
  theme: defaultTheme({
    logo: '/logo.svg',
    logoDark: '/logo.svg',
    repo: '',
    docsDir: 'docs',
    docsBranch: 'main',
    editLink: false,
    lastUpdated: true,
    contributors: false,
    colorMode: 'auto',
    colorModeSwitch: true,
    navbar: [
      { text: '指南', link: '/guide/installation' },
      { text: 'API 参考', link: '/api/' },
      {
        text: '版本',
        children: [
          { text: '1.x', link: '/' },
        ],
      },
    ],
    sidebar: {
      '/guide/': [
        {
          text: '序言',
          collapsible: false,
          children: [
            { text: '安装', link: '/guide/installation' },
            { text: '配置', link: '/guide/configuration' },
            { text: '目录结构', link: '/guide/structure' },
          ],
        },
        {
          text: '核心概念',
          collapsible: false,
          children: [
            { text: '路由', link: '/guide/routing' },
            { text: '控制器', link: '/guide/controllers' },
            { text: '视图', link: '/guide/views' },
            { text: '数据库', link: '/guide/database' },
          ],
        },
        {
          text: '深入探索',
          collapsible: false,
          children: [
            { text: 'HTTP 层', link: '/guide/http' },
            { text: '会话管理', link: '/guide/session' },
            { text: '缓存', link: '/guide/caching' },
            { text: '日志', link: '/guide/logging' },
            { text: '错误处理', link: '/guide/error-handling' },
          ],
        },
        {
          text: '工具集',
          collapsible: false,
          children: [
            { text: '图像处理', link: '/guide/images' },
            { text: '验证', link: '/guide/validation' },
            { text: '辅助函数', link: '/guide/helpers' },
            { text: '外观模式', link: '/guide/facades' },
            { text: '安全', link: '/guide/security' },
          ],
        },
      ],
      '/api/': [
        {
          text: '框架核心',
          collapsible: false,
          children: [
            { text: 'App', link: '/api/app' },
            { text: 'Config', link: '/api/config' },
            { text: 'ErrorHandler', link: '/api/error-handler' },
          ],
        },
        {
          text: 'HTTP',
          collapsible: false,
          children: [
            { text: 'Router', link: '/api/router' },
            { text: 'Route', link: '/api/route' },
            { text: 'Controller', link: '/api/controller' },
            { text: 'Request', link: '/api/request' },
            { text: 'Response', link: '/api/response' },
            { text: 'Session', link: '/api/session' },
            { text: 'Middleware', link: '/api/middleware' },
          ],
        },
        {
          text: '缓存',
          collapsible: false,
          children: [
            { text: 'CacheInterface', link: '/api/cache-interface' },
            { text: 'FileCache', link: '/api/file-cache' },
            { text: 'RedisCache', link: '/api/redis-cache' },
            { text: 'MemcacheCache', link: '/api/memcache-cache' },
          ],
        },
        {
          text: '视图',
          collapsible: false,
          children: [
            { text: 'View', link: '/api/view' },
            { text: 'PHPRenderer', link: '/api/php-renderer' },
            { text: 'TwigViewRenderer', link: '/api/twig-renderer' },
          ],
        },
        {
          text: '数据库',
          collapsible: false,
          children: [
            { text: 'DB', link: '/api/db' },
            { text: 'ZPDO', link: '/api/zpdo' },
            { text: 'Query', link: '/api/query' },
            { text: 'Model', link: '/api/model' },
          ],
        },
        {
          text: '工具类',
          collapsible: false,
          children: [
            { text: 'Arr & Str', link: '/api/arr-str' },
            { text: 'Date & UUID', link: '/api/date-uuid' },
            { text: 'Password & Random', link: '/api/password-random' },
          ],
        },
        {
          text: '其他',
          collapsible: false,
          children: [
            { text: 'Image', link: '/api/image' },
            { text: 'Log', link: '/api/log' },
            { text: 'Facades', link: '/api/facades' },
            { text: 'Exceptions', link: '/api/exceptions' },
          ],
        },
      ],
    },
    sidebarDepth: 2,
  }),

  lang: 'zh-CN',
  title: 'Zap PHP Framework',
  description: 'Zap PHP Framework 文档 — 轻量级、高性能 PHP 开发框架',
  head: [
    ['meta', { name: 'theme-color', content: '#6366f1' }],
    ['link', { rel: 'icon', href: '/logo.svg' }],
  ],

  markdown: {
    headers: {
      level: [2, 3, 4],
    },
  },
})
